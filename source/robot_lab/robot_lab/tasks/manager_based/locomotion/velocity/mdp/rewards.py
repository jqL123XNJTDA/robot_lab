...
def track_lin_vel_y_exp(
    ...
    """只跟踪Y方向线速度命令的奖励（使用指数核）。

    适用于只能侧向移动的双轮腿机器人。
    """
    # extract the used quantities (to enable type-hinting)
    asset: RigidObject = env.scene[asset_cfg.name]
    # 只计算Y方向的速度误差（command[:, 1] 是 lin_vel_y）
    lin_vel_y_error = torch.square(env.command_manager.get_command(command_name)[:, 1] - asset.data.root_lin_vel_b[:, 1])
    reward = torch.exp(-lin_vel_y_error / std**2)
    reward *= torch.clamp(-env.scene["robot"].data.projected_gravity_b[:, 2], 0, 0.7) / 0.7
    return reward
...
def handstand_calf_height_linear(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
    ...
    Returns:
        torch.Tensor: 奖励值 (batch_size,) - 高度在 [min_height, max_height] 范围内线性增长
