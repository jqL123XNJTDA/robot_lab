...
"""
播放和导出 HIM (History-based Implicit Model) 策略。

用于测试训练好的 HIM 策略并导出为 TorchScript JIT 和 ONNX 格式用于部署。

使用方法:
    python scripts/reinforcement_learning/rsl_rl/play_him.py \
        --task RobotLab-Isaac-Velocity-Flat-Helios-Leg-Hist-v0 \
        --load_run 2025-01-01_12-00-00 \
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg):
    ...
    if args_cli.export:
        ...
        print(f"\n[INFO] Exporting HIM policy to: {export_model_dir}")
        ...
        try:
            ...
            print(f"\n[WARNING] Failed to export policy: {e}")
            import traceback
            traceback.print_exc()
