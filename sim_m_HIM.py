#!/usr/bin/env python3
"""
MyDog (四足轮腿机器人) MuJoCo Deployment Script - HIM Version
...
- 12 leg joints (position control): FR/FL/RR/RL × (hip, thigh, calf)
- 4 wheel joints (velocity control): FR/FL/RR/RL × foot

Observation Dimension (HIM: 285D = 57D × 5帧):
每帧 57D:
- base_ang_vel: 3 (angular velocity, scale=0.25)
- projected_gravity: 3
- velocity_commands: 3
- joint_pos: 16 (wheel positions = 0)
- joint_vel: 16 (scale=0.05)
- actions: 16 (previous action)
...
- Leg position control (12D): FR/FL/RR/RL × (hip, thigh, calf)
- Wheel velocity control (4D): FR/FL/RR/RL × foot

Usage:
    python "sim_m HIM.py" --policy-path <policy_path> [--keyboard]
"""
...
# ============================================================================
# HIM Configuration - 历史观测配置 (MyDog 四足轮腿机器人)
# ============================================================================
HISTORY_LEN = 5  # HIM 使用5帧历史
OBS_DIMS = [3, 3, 3, 16, 16, 16]  # [ang_vel, gravity, cmd, jpos, jvel, act]
OBS_PER_FRAME = sum(OBS_DIMS)  # 57D per frame
TOTAL_OBS_DIM = OBS_PER_FRAME * HISTORY_LEN  # 285D total


def reshape_isaac_to_him(obs_flat: np.ndarray, history_len: int, obs_dims: list) -> np.ndarray:
    """
    将 Isaac Lab 观测格式转换为 HIM 格式。

    Isaac Lab: [var1(t-4...t-0), var2(t-4...t-0), ...]  按变量分组
    HIM: [all_vars(t-4), all_vars(t-3), ..., all_vars(t-0)]  按时间步分组

    Args:
        obs_flat: Isaac Lab 展平观测 [total_dims]
        history_len: 历史长度
        obs_dims: 每个变量的维度列表

    Returns:
        重排序后的 HIM 格式观测 [total_dims]
    """
    # 提取每个变量的历史
    var_histories = []
    offset = 0
...
class ObservationHistory:
    ...
    def __init__(self, history_len: int = 5, obs_dims: list = None):
        ...
        self.obs_dims = obs_dims if obs_dims else [3, 3, 3, 16, 16, 16]
...
def get_obs(data, vel_cmd, last_action, debug=False):
    ...
    HIM 观测顺序（与 Isaac Lab 训练一致）：
    - [0:3] base_ang_vel: 基座角速度 (scale=0.25)
    - [3:6] projected_gravity: 投影重力
    - [6:9] velocity_commands: [vx, vy, vyaw]
    - [9:25] joint_pos: 关节位置 (wheel positions = 0)
    - [25:41] joint_vel: 关节速度 (scale=0.05)
    - [41:57] actions: 上一步动作
    ...
    # 组装单帧观测向量 (57D)
    # 顺序: ang_vel(3), gravity(3), cmd(3), jpos(16), jvel(16), action(16)
    obs = np.concatenate([
...
def run_mujoco(policy, mujoco_model_path, sim_duration, dt, decimation, debug=False, plot=False, keyboard_control=False, realtime_plot=False):
    ...
    if keyboard_control:
        print("\n" + "="*70)
        print("MyDog HIM Keyboard Controls")
        print("="*70)
            ...
            if step % decimation == 0:
                ...
                if pose_ctrl.mode == ControlMode.NORMAL:
                    ...
                    print(f"\r[Gravity] x={proj_gravity[0]:+.3f} y={proj_gravity[1]:+.3f} z={proj_gravity[2]:+.3f} | "
                          f"[Height] {base_height:.3f}m | [Cmd] vx={vel_cmd.vx:.2f} vy={vel_cmd.vy:.2f}", end="")
...
if __name__ == '__main__':
    ...
    print("\n" + "="*70)
    print("HIM Policy Configuration (MyDog 四足轮腿机器人)")
    print("="*70)
    print(f"  History length: {HISTORY_LEN} frames")
